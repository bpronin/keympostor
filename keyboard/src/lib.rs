...
mod tests {
    ...
    //     
    //     let actual = ScanCode::by_symbol('~').unwrap();
    //     // must be with SHIFT pressed
    //     assert_eq!("SC_BACKTICK", actual.name);
    // }
    
    #[test]
    fn test_get_sc_ext_value() {
        let actual = ScanCode::by_ext_code(0x1C).unwrap();
