...
impl KeyboardState {
    ...
    pub(crate) fn all_down(&self, keys: &[KeyCode]) -> bool {
        keys.iter().all(|key| self.is_down(key))
    }

    fn is_down(&self, key: &KeyCode) -> bool {
        let vk_code = key.as_virtual_key().unwrap().value;
        self.keys[vk_code as usize] == DOWN_STATE
    }
...
//             keys: VK(VirtualKey::from_code(vk_code as u8).unwrap()),
//             transition: Up,
//         });
